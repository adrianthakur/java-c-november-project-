...
public:
    ...
    friend bool compareMarks(const Student& a, const Student& b);
...
// Display all using polymorphism
void displayAll(const std::vector<Student>& students) {
    Student::header();
    std::vector<Person*> people;
    for (const auto& s : students) people.push_back(new Student(s));
...
void displayTopper(const std::vector<Student>& students) {
    auto top = *std::max_element(students.begin(), students.end(), compareMarks);
    std::cout << "\nTopper:\n";
    Student::header();
    top.display();
}
...
void displayAverage(const std::vector<Student>& students) {
    float total = std::accumulate(students.begin(), students.end(), 0.0f,
        [](float sum, const Student& s) { return sum + s.getMarks(); });
...
void addStudent(std::vector<Student>& students, int roll, std::string name, float marks) {
    students.emplace_back(roll, name, marks);
...
void addStudent(std::vector<Student>& students) {
    ...
    addStudent(students, roll, name, marks);
